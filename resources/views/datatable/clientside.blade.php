                                ...
                                <tbody>
                                    @foreach ($data as $d)
                                        <tr>
                                            ...
                                            <td>
                                                @if(isset($d->image) && $d->image)
                                                    <img src="{{ asset('storage/photo-user/' . $d->image) }}" alt="User Image"
                                                        width="100">
                                                @else
                                                    <img src="{{ asset('storage/photo-user/default.png') }}" alt="No Image"
                                                        width="100">
                                                @endif
                                            </td>
                                            <td>{{ $d->name }}</td>
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <p>Apakah kamu yakin ingin menghapus data user <b>{{$d->name}}</b>?
                                                        </p>
                                                    </div>
                                                    <div class="modal-footer justify-content-between">
                                                        <form action="{{ route('admin.delete', ['id' => $d->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
